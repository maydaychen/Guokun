...
import com.example.user.guokun.adapter.GoodsDaizhifuAdapter;
...
public class DaizhifuFragment extends Fragment implements PullLoadMoreRecyclerView.PullLoadMoreListener {
    ...
    private GoodsDaizhifuAdapter mRecyclerViewAdapter;
    private int page = 1;
    ...
    public static DaizhifuFragment newInstance(String content) {
        Bundle arguments = new Bundle();
        arguments.putString(EXTRA_CONTENT, content);

        DaizhifuFragment mallFragment = new DaizhifuFragment();
    ...
    public void initData() {
        ...
        mRecyclerViewAdapter = new GoodsDaizhifuAdapter(getActivity());
        ...
        switch (payMessage.getStatus()) {
            case "canl":
                HttpShopMethod.getInstance().change_order(new ProgressSubscriber<>(changeOnNext,
                        getActivity()), mPreferences.getString("access_token", ""), "", mPreferences.getString("sessionkey", ""), payMessage.getId(), "canl");
                break;
        }
    ...
    private void getData1() {
        HttpShopMethod.getInstance().orders(new ProgressErrorSubscriber<>(VspaOnNext,
                getActivity()), mPreferences.getString("access_token", ""), mPreferences.getString("sessionkey", ""), page, "0");
    }
}
